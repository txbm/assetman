...
class Assetman(object):
	...
	def connection(self):
		if not self._connection:
			self._connection = s3.get_connection(self.aws_key, self.aws_secret)
		return self._connection

	def save_asset(self, asset):
		bucket = s3.get_bucket(self.bucket_name, self.connection)
	...
	def get_asset(self, uuid):
		...
		asset = assets.lookup_by_type(key.get_metadata('asset_type'))(key.get_contents_as_string())
	...
	def delete_asset(self, uuid):
		bucket = s3ops.get_bucket(self.bucket_name, self.connection)
